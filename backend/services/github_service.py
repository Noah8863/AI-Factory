...
# Full workflow: backend checks + frontend build + GitHub Pages deploy.
# Used when the project has frontend tickets.
_CI_WORKFLOW_YAML = """\
...
# Backend-only workflow: no GitHub Pages deploy job.
# Used when the project has no frontend tickets.
_CI_WORKFLOW_YAML_BACKEND_ONLY = """\
# Auto-generated by AI Factory
name: CI / CD
...
def deploy_ci_workflow(repo_name: str, has_frontend: bool = True) -> bool:
    """
    Write the GitHub Actions CI/CD workflow into the repo on the main branch.
    Also enables GitHub Pages when the project has frontend code.

    has_frontend — True  → full workflow with GitHub Pages deploy job.
                   False → backend-only workflow; Pages is not enabled.

    Should only be called after ALL tickets are done (no pending / failed).
    Returns True if the workflow file was successfully committed.
    """
    if has_frontend:
        ...
        workflow_yaml = _CI_WORKFLOW_YAML
    else:
        print(f"ℹ️  No frontend tickets for {repo_name} — skipping GitHub Pages, using backend-only workflow.")
        workflow_yaml = _CI_WORKFLOW_YAML_BACKEND_ONLY
